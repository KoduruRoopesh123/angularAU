...
@Component({
  selector: 'app-auto-debit-request',
  templateUrl: './auto-debit-request.component.html',
  styleUrls: ['./auto-debit-request.component.css']
})
export class AutoDebitRequestComponent implements OnInit {
  ...
  constructor(private regCust: CustRegService, 
    ...
    basicDataGet(){
      var autoDebitTrans = { action: 'autoDebitTrans', tabid:localStorage.getItem('tabid') };

      this.regCust.custRegValidation2(autoDebitTrans).subscribe(
        (response) => {
    ...
    this.dtOptions = {
      ...
      dom: 'Bfrtip',
        buttons: [
          ...
          { extend: 'excel',text:'Excel',title:'AutoDebitFileUpoloadReport',exportOptions:{columns:[0,1,2,3,4,5]}, className: 'btn btn-info' },
          // {
  ...
  open(content:any,report:any) {
   ...
   reportFilter(){
    ...
    this.myForm.controls['action'].setValue('autoDebitTrans');
