...
@Component({
  selector: 'app-idam-report',
  templateUrl: './idam-report.component.html',
  styleUrls: ['./idam-report.component.css']
})
export class IdamReportComponent implements OnInit {
  ...
  constructor(private regCust: CustRegService, 
    ...
    basicDataGet(){
      var idamReport = { action: 'idamReport', tabid:localStorage.getItem('tabid')};
  
      this.regCust.custRegValidation2(idamReport).subscribe(
        (response) => {
      ...
      this.dtOptions = {
        ...
        dom: 'Bfrtip',
          buttons: [
            ...
            { extend: 'excel',text:'Excel',title:'idamRequestReport',exportOptions:{columns:[0,1,2,3]}, className: 'btn btn-info' },
            //   { 
  ...
  open(content:any,report:any) {
   ...
   reportFilter(){
    ...
    this.myForm.controls['action'].setValue('idamReport');
